...
if not hasattr(sys,'frozen'):
    ...
    # encode properly to MBCS make Python unable to find the path for
    # importing.  This doesn't normally affect standard or 3rd party modules,
    # but it does affect any module provided with said application/module.  To
    # work around this, we install an import hook that accesses the appropriate
    # file directly and loads it into Python.
        ...
        def load_module(self,fullname):
            ...
            if os.path.exists(filename+'.py'):
                try:
                    ...
                    print('UnicodeImporter failed importing %s: %s'
                          % (filename+'.py',e))
                    if fullname in sys.modules:
